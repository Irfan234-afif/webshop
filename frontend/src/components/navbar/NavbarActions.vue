...
<template>
  <div class="flex items-center space-x-4">
    ...
    <!-- Wishlist Button -->
    <RouterLink v-if="authStore.isAuthenticated" to="/wishlist"
      class="p-2 text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors relative"
      aria-label="Wishlist">
      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <!-- Cart Button -->
    <button v-if="authStore.isAuthenticated" @click="$emit('openCart')"
      class="p-2 text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors relative"

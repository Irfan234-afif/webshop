...
const payBill = (bill: any) => {
    // Redirect to standard payment page or trigger payment modal
    // For now, assuming standard ERPNext payment flow url or custom checkout
    // A simple way is to redirect to the generic payment-request handler if available, 
    // or just show an alert that this part needs integration.
    // Let's assume we want to integrate with the same modal used in OrdersPage eventually.
    // For MVP, lets just log.
    console.log("Pay bill", bill)
    // window.location.href = `/payment-request?reference_doctype=Sales%20Invoice&reference_name=${bill.name}`
    alert("Fitur Pembayaran Tagihan akan segera hadir. Silakan hubungi admin.")
}
</script>

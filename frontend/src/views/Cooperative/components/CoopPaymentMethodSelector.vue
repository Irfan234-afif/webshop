...
const { paymentMethods, paymentMethodType, paymentChannel } = storeToRefs(cooperativeStore)
...
watch(selectedMethod, (newVal) => {
  if (newVal) {
    cooperativeStore.paymentMethodType = newVal
  }
})

onMounted(async () => {
  if (paymentMethods.value.length === 0) {
    await cooperativeStore.fetchPaymentMethods()
  }
})
...
function selectPaymentMethod(methodName: string) {
  ...
  if (selectedMethod.value !== methodName) {
    ...
    cooperativeStore.paymentChannel = ''
  }

  selectedMethod.value = methodName
  cooperativeStore.paymentMethodType = methodName
}

function selectChannel(channelCode: string) {
  selectedChannel.value = channelCode
  cooperativeStore.paymentChannel = channelCode
}

    ...
    <div class="min-h-screen bg-gray-50 py-12">
      <Container>
        <div class="max-w-3xl mx-auto">
          <!-- Back Link -->
          <router-link to="/member" class="inline-flex items-center text-gray-500 hover:text-gray-700 mb-6 transition-colors">
            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
            Kembali ke Dashboard Keanggotaan
          </router-link>
          ...
          <div v-else-if="paymentDetails" class="space-y-6">
            <div class="flex items-center gap-3 mb-6 pl-2">
              ...
              <div>
                ...
                <p class="text-gray-500">Selesaikan pembayaran untuk mengaktifkan keanggotaan Anda</p>
              </div>
            </div>

            <!-- Virtual Account View -->
            <VirtualAccountView 
              v-if="isVirtualAccount" 
...
import { getCooperativePaymentDetails, uploadCoopPaymentProof } from '@/utils/cooperativeApi'
...
const fetchPaymentDetails = async () => {
  ...
  try {
    // 1. Ensure we have the member name
    await cooperativeStore.fetchMembershipStatus()
    const memberName = cooperativeStore.membershipStatus?.name
    
    if (!memberName) {
      throw new Error("Anda belum terdaftar atau data tidak ditemukan.")
    }

    // 2. Wrap the API fetching the mock CheckPaymentDetails format
    paymentDetails.value = await getCooperativePaymentDetails(memberName)
...
const handleUploadSuccess = async (fileUrl: string, notes?: string) => {
  ...
  try {
    ...
    await uploadCoopPaymentProof(memberName, fileUrl, notes)
    
    // Refresh the details to get the updated status
    await fetchPaymentDetails()
  } catch (err: any) {
    console.error('Failed to update payment proof:', err)
    alert(extractErrorMessage(err) || 'Gagal menyimpan bukti pembayaran')
  }
}

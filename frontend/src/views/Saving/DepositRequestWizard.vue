    ...
    <Container class="py-4 md:py-6 lg:py-8">
      <div class="max-w-2xl mx-auto">
        ...
        <div class="mb-8">
          ...
          <h1 class="text-3xl font-bold text-gray-900">Setoran Simpanan Sukarela</h1>
          <p class="text-gray-600 mt-2">
            Isi nominal dan pilih metode pembayaran untuk setoran simpanan sukarela Anda.
          </p>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-xl p-8 mb-6">
          ...
          <div v-if="currentStep === 1">
            <h2 class="text-xl font-bold text-gray-900 mb-6">Nominal Setoran</h2>
            <div class="space-y-4">
              <div>
                ...
                <p v-else class="text-xs text-gray-500 mt-2">Minimal setoran Rp 10.000</p>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="currentStep === 3">
            <h2 class="text-xl font-bold text-gray-900 mb-6">Konfirmasi Setoran</h2>
            ...
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-200 space-y-4">
              <div class="flex justify-between py-2 border-b border-gray-200">
                <span class="text-gray-600">Nominal Setoran</span>
        ...
        <div class="flex items-center justify-between gap-4">
          ...
          <button v-else @click="submitDeposit" type="button" :disabled="isSubmitting"
...
import { createVoluntarySavingPayment } from '@/utils/savingPaymentApi'
...
const steps = [
  { number: 1, title: 'Nominal Setoran' },
...
const paymentMethodTitle = computed(() => {
  const method = refundPaymentMethods.value.find((m: any) => m.name === formData.value.refund_payment_mode)
...
function validateAmount() {
  ...
  if (amt < 10000) {
    amountError.value = "Minimal setoran adalah Rp 10.000";
...
function updateBankDetails(details: any) {
  // Not needed for deposit
}
...
async function submitDeposit() {
  try {
    ...
    const response = await createVoluntarySavingPayment(
      formData.value.amount,
      formData.value.refund_payment_mode
    )

    if (response.payment_request) {
      alertStore.success('Pengajuan setoran berhasil!', 'Success')

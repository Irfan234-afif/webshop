...
<template>
  <div class="flex flex-col gap-4 sticky top-20 sm:top-24 lg:top-28">
    ...
    <!-- Checkout Button -->
    <button @click="handleCheckout" :disabled="items.length === 0 || isCheckoutLoading"
      ...
      <svg v-if="isCheckoutLoading" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white"
      ...
      {{ isCheckoutLoading ? 'Memproses...' : 'Buat Pesanan' }}
    </button>
  </div>
</template>

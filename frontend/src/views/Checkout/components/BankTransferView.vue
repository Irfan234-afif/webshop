...
<template>
  <div class="bank-transfer-view max-w-3xl mx-auto px-4">
    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-8 md:p-12 shadow-sm">
      ...
      <div
        v-if="bankDetails"
        class="bg-gray-50 border border-gray-200 rounded-xl p-6 flex items-center gap-4 mb-4"
      >
        ...
        <button
          @click="copyToClipboard(bankDetails.account_number)"
      ...
      <div class="bg-gray-50 border border-gray-200 rounded-xl p-6 flex items-center gap-4 mb-6">
        ...
        <button
          @click="copyToClipboard(paymentDetails.sales_order.grand_total.toString())"
          class="flex-shrink-0 p-2 hover:bg-gray-200 rounded-lg transition-colors"
          title="Copy amount"
        >
          <!-- Copy Icon -->
      ...
      <div v-if="paymentApproval?.status === 'Approved'" class="bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-200 rounded-2xl p-8 mb-6 shadow-lg">
        ...
        <div class="bg-white rounded-xl p-6 mb-4 border border-green-100">
          ...
          <div class="space-y-3">
            ...
            <div class="flex justify-between items-center py-2 border-b border-gray-100">
              ...
              <span class="text-sm font-bold text-green-700">{{ formatCurrency(paymentDetails.sales_order.grand_total) }}</span>
            </div>
            <div class="flex justify-between items-center py-2 border-b border-gray-100">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div class="flex-1">
            ...
            <p class="text-sm text-blue-700">
              Pesanan Anda akan segera diproses oleh tim kami. Anda dapat melihat status pesanan di halaman riwayat pembelian.
            </p>
          </div>
        </div>
        ...
        <details v-if="hasUploadedProof" class="mt-4 bg-white border border-green-100 rounded-lg">
          <summary class="cursor-pointer p-4 font-medium text-sm text-gray-700 hover:bg-gray-50 rounded-lg transition-colors">
            Lihat Bukti Transfer yang Diupload
          </summary>
          <div class="p-4 pt-0">
            <div v-if="isImageFile(paymentApproval?.payment_proof)" class="rounded-lg overflow-hidden border border-gray-200 mt-2">
              <img
                :src="paymentApproval?.payment_proof || ''"
                alt="Bukti Transfer"
            ...
            <a
              v-else
              :href="paymentApproval?.payment_proof || ''"
              target="_blank"
      ...
      <div v-else-if="hasUploadedProof" class="bg-white border border-gray-200 rounded-xl p-6 mb-6">
        ...
        <div class="mb-4">
          <div v-if="isImageFile(paymentApproval?.payment_proof)" class="rounded-lg overflow-hidden border border-gray-200">
            <img
              :src="paymentApproval?.payment_proof || ''"
              alt="Bukti Transfer"
              class="w-full h-auto max-h-96 object-contain bg-gray-50"
        ...
        <!-- Status Message -->
        <div v-if="paymentApproval?.status === 'Pending'" class="flex items-start gap-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <svg class="w-5 h-5 text-yellow-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <p class="text-sm text-yellow-800">
            Bukti transfer Anda sedang dalam proses verifikasi oleh admin. Anda akan mendapatkan notifikasi setelah disetujui.
          </p>
        </div>

        <div v-else-if="paymentApproval?.status === 'Rejected'" class="flex items-start gap-2 p-3 bg-red-50 border border-red-200 rounded-lg">
      ...
      <div
        v-if="showPaymentDeadline"
        class="bg-white border border-t-0 border-gray-200 rounded-b-xl p-6 mb-6"
      >
